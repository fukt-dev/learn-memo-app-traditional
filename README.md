...
以下のスクリーンショットは、Flywayのシードデータ（`V2__Seed_sample_memos.sql`）で投入されるサンプルメモを表示した状態です。

![メモ一覧画面](docs/images/memo-list.png)

*メモ一覧画面。登録済みのメモが作成日時の新しい順に並び、検索ボックスと「新規作成」ボタンがある*
...
![メモ新規作成画面](docs/images/memo-form.png)

*メモ新規作成画面。タイトルと本文の入力欄のみのシンプルなフォーム*

### メモの表示

- 一覧画面で任意のメモのタイトルをクリック

![メモ詳細画面](docs/images/memo-detail.png)

*メモ詳細画面。作成日時・更新日時とともに本文全体を表示する*
...
![メモ編集画面](docs/images/memo-edit.png)

*メモ編集画面。詳細画面で選択したメモの内容がフォームに初期表示される*
